...
async function wrapper(){
	...
	function init() {
		...
		for ( var i = 0; i < data.data.length; i ++ ){
			...
			if (data.data[i].l[0] < pca1_min){
				pca1_min = data.data[i].l[0]
			}
		}
	    ...
	    camera.lookAt(scene.position);
	 	...
	 	loader.load("/static/twitter_network/src/helvetiker.json", function(text){
	 		...
	 		var textGeo = new THREE.TextGeometry(c[1], {
				...
				centroidLabel.position.set( - spreadMult - 25, cIndex * 10 , 0)
				centroidSphere.position.set(- spreadMult - 25, cIndex * 10 , 0)
			...
			var linepoints = [];
			linepoints.push( new THREE.Vector3( -spreadMult, pca1_min , 0 ) );
			linepoints.push( new THREE.Vector3( spreadMult, pca1_min  , 0 ) );
			...
			for(var day=0; day < Difference_In_Days; day++){
				var tickday = addDays(timeExt[0], day)
				var tickpoints = []
				tickpoints.push( new THREE.Vector3( timeScale(tickday), pca1_min , 0 ) );
				tickpoints.push( new THREE.Vector3( timeScale(tickday), (pca1_min  ) - 2 , 0 ) );
				var tickgeometry = new THREE.BufferGeometry().setFromPoints( tickpoints );
				var tickline = new THREE.Line( tickgeometry, linematerial );
				scene.add(tickline)
				var ticktextMat = new THREE.MeshBasicMaterial( {
						color: 0x00000,
						transparent: true,
						opacity: 1,
					} );
				
	 			var ticktextGeo = new THREE.TextGeometry(dateFormat(tickday), {
	 				font : text,
	 				size: .5,
	 				height: 0,
	 				bevelThickness : 1,
	 			...
	 			ticktextGeo.center()
	 			let tickLabel = new THREE.Mesh(ticktextGeo, ticktextMat)
	 			tickLabel.position.set( timeScale(tickday), (pca1_min  ) - 3 , 0 )
				scene.add(tickLabel)
			}
	 	})//loader
